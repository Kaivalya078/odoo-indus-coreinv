...
export default function Receipts() {
  ...
  const action = async (id, act) => {
    try { setError(''); await api.post(`/receipts/${id}/${act}`); load() }
    catch (err) { setError(err.response?.data?.detail || 'Error') }
  }
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Receipts</h1>
        <button className="btn btn-primary" onClick={() => setShow(true)}><Plus size={16} /> New Receipt</button>
      </div>
      ...
      <table>
        ...
        <tbody>
          {items.map(r => (
            <tr key={r.id}>
              <td className="font-bold">{r.reference}</td><td>{r.supplier_name || '—'}</td><td>{r.warehouse_name}</td>
          ...
          {items.length === 0 && <tr><td colSpan={7} className="text-muted" style={{ textAlign: 'center', padding: 32 }}>No receipts</td></tr>}
        </tbody>
      </table>
      {show && (
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={submit}>
              ...
              <div className="actions" style={{ marginTop: 8 }}>
                <button className="btn btn-primary" type="submit">Create Receipt</button>
