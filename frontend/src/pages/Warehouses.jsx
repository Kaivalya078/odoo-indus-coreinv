...
export default function Warehouses() {
  ...
  const remove = async (id) => { await api.delete(`/warehouses/${id}`); load() }

  return (
    <div>
      <div className="page-header">
        <h1>Warehouses</h1>
        <button className="btn btn-primary" onClick={() => setShow(true)}>+ New Warehouse</button>
      </div>
      <table>
        ...
        <tbody>
          ...
          {items.length === 0 && <tr><td colSpan={3}>No warehouses</td></tr>}
        </tbody>
      </table>
      {show && (
